...
房间的房主可以与超过默认 6 名玩家（目标的房间上限为 **16**（EOS 会话硬上限））一起游玩，并为
7 人以上游玩时的主要痛点提供房主工具：聚拢走散的玩家、重载卡住的关卡、
登上电梯、将已知会随人数增长的过关要求封顶为 6，以及在关卡并非为这么多人设计时帮你挤过地形。

> **仅房主需要安装。** 只有房间房主/监听服务器需要这个模组。朋友们可以
> 用未修改的游戏正常加入。
>
...
- 有些关卡并非为 7 人以上设计。过关要求封顶会处理游戏中标记为
  `bScalesWithPlayers` 的人数缩放目标，以及已知的电梯人数闸门和 Level 1
  发电机数量等字段；但地形/加载问题仍可能需要变通。电梯和穿墙工具并不是官方关卡支持。

...
```powershell
Get-FileHash .\BETPlayerCap-v2.17.0-full.zip -Algorithm SHA256
```
...
This regenerates `dist/BETPlayerCap-v2.17.0-full.zip`. Then refresh the checksum:

```bash
sha256sum dist/BETPlayerCap-v2.17.0-full.zip | sed 's#dist/##' > dist/BETPlayerCap-v2.17.0-full.zip.sha256
```

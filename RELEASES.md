...
- `BETPlayerCap-v2.19.7-full.zip`

## Verify your download (recommended)

A SHA256 checksum is tracked in this repo at
[`dist/BETPlayerCap-v2.19.7-full.zip.sha256`](dist/BETPlayerCap-v2.19.7-full.zip.sha256)
and is also posted in the release notes.

PowerShell:

```powershell
Get-FileHash .\BETPlayerCap-v2.19.7-full.zip -Algorithm SHA256
```
...
This regenerates `dist/BETPlayerCap-v2.19.7-full.zip`. Then refresh the checksum:

```bash
sha256sum dist/BETPlayerCap-v2.19.7-full.zip | sed 's#dist/##' > dist/BETPlayerCap-v2.19.7-full.zip.sha256
```
